...
/**
 * @brief popluate vector according to r_type and neg_type 
 * @param {string} r_type 返回类型 "COMMMON" 使用相同key生成vector, "INDEF" 使用独特的key生成vector
 * @param {string} neg_type 是否生成相反数
 * @return 返回vec,生成随机向量
 */
template <typename T>
void populateRandomVector(vector<T> &vec, size_t size, string r_type, string neg_type)
{
  ...
  if (r_type == "COMMON")
  {
    unsigned long tmp = aes_common->get64Bits();
...
template <typename T>
/*** 
 * @brief input shares of x in L-1
 * @param {vector<T>} x
 * @return {vector<myType>} shares_1
 * @return {vector<myType>} shares_2
 */
void sharesModuloOdd(vector<myType> &shares_1, vector<myType> &shares_2,
                     const vector<T> &x, size_t size, string r_type)
{
...
template <typename T1, typename T2>
/*** 
 * @brief a，b 同时为环的域时，返回0 否则计算在计算环上的运算结果
 * @param {T1} a
 * @param {T2} b
 * @return {myType} (a+b)%(module-1)
 */
myType addModuloOdd(T1 a, T2 b)
{
  assert((sizeof(T1) == sizeof(myType) or sizeof(T2) == sizeof(myType)) && "At least one type should be myType for typecast to work");
