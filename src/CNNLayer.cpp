...
/**
 * @description: 
 * @param {*} 初始化权重和偏置值,权重值都设为1，偏置值都设置为0
 * @return {*}
 */
...
/*** 
 * @brief compute activations, 
 * @param {vector<myType>} &inputActivation
 */
void CNNLayer::forward(const vector<myType> &inputActivation)
{
  log_print("CNN.forward");
  // B为batchSize 一次训练所选取的样本数
  ...
  //Reshape weights，改变权重的shape
  // shape of weights (filterWidth*filterHeight*inputFeatures,filters)
  ...
  //reshape activations
  // printVector("weights", weights, weights.size());
  size_t size_convo = (p_range * q_range * B) * (fw * fh * C);
  vector<myType> convShaped(size_convo, 0);
  //将 activations 的shape转化为 convshaped
  convolutionReshape(inputActivation, convShaped, iw, ih, C, B, fw, fh, 1, 1);

  //Convolution multiplication 进行卷积，将计算结果取整，如果是单方直接进行矩阵乘法，多方调用多方矩阵乘法

  //convOutput=activation*weights+biases
  ...
  if (STANDALONE)
  {
    ...
    {
      // 判断y是否大于0 lageest_NEG = 0x8000000000000000 if y > 0 return 1 if y < 0 return 0
  ...
  if (THREE_PC)
  {
    // 根据y 给出reluPrimeLarge的取值， ruleprimeLarge = 0 or 1
    funcRELUPrime3PC(y, reluPrimeLarge, size_x / (px * py));
    // activtions = y*reluprimeLarge = y or 0
...
/*** 
 * @description: 对于单方和多方都能支持查找池中最大值及其索引
 * @param {matrix} a 
 * @param {size_t} rows
 * @param {size_t} colums
 * @return {vector} max {vector} maxIndex
 */
...
void CNNLayer::computeDeltaSA(vector<myType> &prevDelta)
{
  ...
  printVector("deltas", deltas, deltas.size());
  ...
  for (size_t i = 0; i < size_w; ++i)
    reshapedWeights[((i / size_D) * size_D) + ((i % size_C) * C) + ((i % size_D) / size_C)] = weights[(i / size_C) * size_C + size_C - 1 - (i % size_C)];
...
/*** 
 * @description: standalone max,计算每个行向量中的最大值
 * @param {matrix} a,存储方式为先行后列
 * @param {rows} rows
 * @param {columns} columns
 * @return {max,maxIndex} max,maxIndex
 */
...
/*** 
 * @brief 求导
 * @return {vector<myType>} &prevDelta
 */
